...
const RotatingShape: React.FC = () => {
  ...
  useFrame((state, delta) => {
    if (meshRef.current) {
      ...
      // @ts-expect-error originalPositions is a custom property we add to geometry at runtime
      if (positions && meshRef.current.geometry.originalPositions) {
        const count = positions.count;
        const step = 1; // Fix: define step
        // Apply distortion - limit iterations for better performance
        for (let i = 0; i < count; i += step) {
          // @ts-expect-error originalPositions is a custom array property added at runtime
  ...
  const currentGeometry = useMemo(() => {
    ...
    try {
      ...
      if (geometry && geometry.attributes.position) {
        ...
        // Extend geometry type to include originalPositions
        interface CustomGeometry extends THREE.BufferGeometry {
          originalPositions?: number[];
        }
        (geometry as CustomGeometry).originalPositions = Array.from(positions);
      }
    } catch (error) {
  ...
  return (
    <a.mesh
      ...
      key={`geometry-${geometryIndex}`}
      geometry={currentGeometry}
    >
      <a.meshPhysicalMaterial
        color={hovered ? '#ffcd00' : '#c490ff'}
        clearcoat={shine.to((s: number) => s * 1.2)}
        clearcoatRoughness={0.05}
        metalness={shine.to((s: number) => s * 0.8)}
        roughness={shine.to((s: number) => 0.05 + (0.1 * (1 - s)))}
        transmission={0.2}
        transparent={true}
        opacity={0.95}
        envMapIntensity={shine.to((s: number) => s * 3)}
        reflectivity={shine.to((s: number) => s * 1.5)}
        iridescence={0.3}
        iridescenceIOR={1.5}
        sheen={0.5}
        sheenRoughness={0.2}
  ...
  return (
    ...
    >
      <div 
        ...
        className={`w-full h-full relative ${allowScroll ? 'allow-scroll' : 'prevent-scroll'}`}
      >
        <ErrorBoundary>
          ...
          <Canvas 
            ...
            dpr={typeof window !== 'undefined' ? Math.min(1.5, window.devicePixelRatio) : 1.5} // SSR safe
