...
type SectionName = 'home' | 'skills' | 'experience' | 'projects' | 'contact';
...
const Navbar: React.FC = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections: SectionName[] = ['home', 'skills', 'experience', 'projects', 'contact'];
          ...
          <div className="hidden md:flex items-center space-x-1 lg:space-x-8">
            {navLinks.map((link) => (
              <button
                ...
                className={`relative py-2 px-3 text-sm lg:text-base font-medium cursor-pointer transition-all duration-300 ${activeSection === link.to ? 'text-primary' : 'text-gray-300 hover:text-gray-100'}`}
              >
                {link.name}
            ...
            <button
              ...
              className="bg-gradient-to-r from-primary to-secondary text-dark-800 px-4 lg:px-6 py-2 rounded-full text-sm lg:text-base font-medium hover:shadow-lg hover:shadow-primary/20 transition-all duration-300"
            >
              Get In Touch
            </button>
          </div>
              ...
              >
                {navLinks.map((link) => (
                  <button
                    ...
                    className={`py-3 px-4 rounded-lg text-center transition-colors w-full ${activeSection === link.to ? 'bg-dark-600 text-primary' : 'text-gray-300'}`}
                  >
                    {link.name}
                  </button>
                ))}
